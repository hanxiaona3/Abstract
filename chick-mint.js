...
async function claimMOCKToken(wallet){
    const Interacted_contract_Token='0x562590e7bae133B6c6e52CF92235499b7d53cD9b';
    ...
    let numberValue = Number("1e-7"); 
    let fixedString = numberValue.toFixed(10); 
    let value1=ethers.parseEther(fixedString);
    // let value1=ethers.parseEther(0.0000001);
    let txData = {
        to: Interacted_contract_Token, 
        data: `${address.substring(2)}`,
        value: value1
    };
...
const main=async(privateKeys)=>{
    ...
    for (let index = 0; index < privateKeys.length; index++) {
        ...
        console.log(`第${index}地址：${morphl2_wallet.address}`);
